<?php
/**
 * Tevah functions, definitions, filters and actions.
 *
 * @package Tevah Lite
 */
...
function toivo_lite_setup() {
	...
	*/
	 ...
	 * If you're building a theme based on Tevah Lite, use a find and replace
	 * to change 'tevah' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'tevah', get_template_directory() . '/languages' );
	...
	register_nav_menus( array( 
		'primary' => _x( 'Primary', 'nav menu location', 'tevah' ),
		'top'     => _x( 'Top', 'nav menu location', 'tevah' ),
		'social'  => _x( 'Social', 'nav menu location', 'tevah' )
	) );
...
function toivo_lite_widgets_init() {
	...
	$sidebar_primary_args = array(
		'id'            => 'primary',
		'name'          => _x( 'Primary', 'sidebar', 'tevah' ),
		'description'   => __( 'The main sidebar. It is displayed on the right or left side of the page.', 'tevah' ),
	...
	$sidebar_subsidiary_args = array(
		'id'            => 'subsidiary',
		'name'          => _x( 'Subsidiary', 'sidebar', 'tevah' ),
		'description'   => __( 'A sidebar located in the footer of the site.', 'tevah' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>'
	...
	$sidebar_front_page_args = apply_filters( 'toivo_sidebar_front_page_args', array(
		'id'            => 'front-page',
		'name'          => _x( 'Front Page', 'sidebar', 'tevah' ),
		'description'   => __( 'A sidebar located in the Front Page Template.', 'tevah' ),
		'before_widget' => '<section id="%1$s" class="widget %2$s">',
		'after_widget'  => '</section>',
		'before_title'  => '<h3 class="widget-title">',
		'after_title'   => '</h3>'
...
function toivo_lite_fonts_url() {
	 ...
	 */
	$lato = _x( 'on', 'Lato font: on or off', 'tevah' );

	/* Translators: If there are characters in your language that are not
	 * supported by Raleway, translate this to 'off'. Do not translate
	 * into your own language.
	 */
	$raleway = _x( 'on', 'Raleway font: on or off', 'tevah' );
...
function toivo_lite_scripts() {
	...
	/* Enqueue fonts. */
	wp_enqueue_style( 'tevah-fonts', toivo_lite_fonts_url(), array(), null );
	...
	if ( is_child_theme() ) {
		wp_enqueue_style( 'tevah-parent-style', trailingslashit( get_template_directory_uri() ) . 'style' . TOIVO_LITE_SUFFIX . '.css', array(), TOIVO_LITE_VERSION );
	}
	
	/* Enqueue active theme styles. */
	wp_enqueue_style( 'tevah-style', get_stylesheet_uri() );
	
	/* Enqueue responsive navigation. */
	wp_enqueue_script( 'tevah-navigation', get_template_directory_uri() . '/js/responsive-nav' . TOIVO_LITE_SUFFIX . '.js', array(), TOIVO_LITE_VERSION, true );
	
	/* Enqueue responsive navigation settings. */
	wp_enqueue_script( 'tevah-settings', trailingslashit( get_template_directory_uri() ) . 'js/settings' . TOIVO_LITE_SUFFIX . '.js', array( 'tevah-navigation' ), TOIVO_LITE_VERSION, true );
	
	/* Enqueue functions. */
	wp_enqueue_script( 'tevah-script', get_template_directory_uri() . '/js/functions' . TOIVO_LITE_SUFFIX . '.js', array(), TOIVO_LITE_VERSION, true );
...
function toivo_lite_excerpt_more() {
	...
	$text = sprintf( __( 'Read more %s', 'tevah' ), '<span class="screen-reader-text">' . get_the_title() . '</span>' );
...
function toivo_lite_nav_description( $item_output, $item, $depth, $args ) {
	if ( 'primary' == $args->theme_location && $item->description ) {
		...
		$item_output = str_replace( $args->link_after . '</a>', '<span class="menu-item-description' . $extra_class . '"><span class="menu-item-description-mark">' . ' ' . _x( '&ndash;' , 'dash before menu item description', 'tevah' ) . '</span> ' . $item->description . '</span>' . $args->link_after . '</a>', $item_output );
	}
...
/**
 * Generate a link to the Tevah theme page.
 *

...
function toivo_lite_create_meta_boxes() {
	...
	foreach ( $toivo_lite_when_to_show_metaboxes as $toivo_lite_when_to_show_metabox ) {
		add_meta_box( 'toivo_lite_metabox', esc_html__( 'Header Callout text', 'tevah' ), 'toivo_lite_meta_box_display', $toivo_lite_when_to_show_metabox, 'normal', 'high' );
	}
...
function toivo_lite_meta_box_display( $post, $metabox ) {
	...
	wp_nonce_field( basename( __FILE__ ), 'toivo-metabox-nonce' ); ?>
		...
		<?php echo sprintf( __( 'Want to replace header text with Callout text and description in singular views? <a href="%s">Upgrade to Tevah (Pro)</a>.', 'tevah' ), esc_url( toivo_lite_get_upgrade_link() ) ); ?>
	</p>

...
function toivo_lite_customize_register( $wp_customize ) {
	...
	$wp_customize->add_panel(
		'theme',
		array(
			'title'       => esc_html__( 'Theme Settings', 'tevah' ),
			'priority'    => 10
		)
	);
	...
	$wp_customize->add_section(
		'tevah-layout',
		array(
			'title'    => esc_html__( 'Layouts', 'tevah' ),
			'priority' => 10,
			'panel'    => 'theme'
		)
	);
	...
	$layout_choices = array( 
		'1c'   => __( '1 Column', 'tevah' ),
		'2c-l' => __( '2 Columns: Content / Sidebar', 'tevah' ),
		'2c-r' => __( '2 Columns: Sidebar / Content', 'tevah' )
	);
	...
	$wp_customize->add_control(
		'theme_layout',
		array(
			'label'    => esc_html__( 'Global Layout', 'tevah' ),
			'section'  => 'tevah-layout',
			'priority' => 10,
			'type'     => 'radio',
	...
	$wp_customize->add_section(
		'front-page',
		array(
			'title'           => esc_html__( 'Front Page Settings', 'tevah' ),
			'description'     => sprintf( __( 'Callout at the top. <a href="%s" target="_blank">Upgrade to Tevah (Pro)</a> for adding similar Callout section at the bottom.', 'tevah' ), esc_url( toivo_lite_get_upgrade_link() ) ),
			'priority'        => 20,
			'panel'           => 'theme',
	...
	/* Add the callout title setting once, in Tevah (Pro) version we have also bottom callout. */
	...
	while ( $k < 1 ) {
		...
		if ( 0 == $k ) {
			$placement_text = _x( 'Top', 'position of callout text', 'tevah' );
		} else {
			$placement_text = _x( 'Bottom', 'position of callout text', 'tevah' );
		}
		...
		$wp_customize->add_control(
			'callout_title_' . $placement,
			array(
				'label'    => sprintf( esc_html__( '%s Callout title', 'tevah' ), $placement_text ),
				'section'  => 'front-page',
				'priority' => 20 + $k*100,
				'type'     => 'text'
			)
		...
		$wp_customize->add_control(
			'callout_text_' . $placement,
			array(
				'label'    => sprintf( esc_html__( '%s Callout text', 'tevah' ), $placement_text ),
				'section'  => 'front-page',
				'priority' => 30 + $k*100,
				'type'     => 'textarea'
			)
		...
		$wp_customize->add_control(
			'callout_url_' . $placement,
			array(
				'label'    => sprintf( esc_html__( '%s Callout URL', 'tevah' ), $placement_text ),
				'section'  => 'front-page',
				'priority' => 50 + $k*100,
				'type'     => 'url'
			)
		...
		$wp_customize->add_control(
			'callout_url_text_' . $placement,
			array(
				'label'    => sprintf( esc_html__( '%s Callout URL text', 'tevah' ), $placement_text ),
				'section'  => 'front-page',
				'priority' => 60 + $k*100,
				'type'     => 'text'
			)
	...
	if( class_exists( 'Jetpack' ) && current_theme_supports( 'jetpack-testimonial' ) ) {
		...
		$wp_customize->add_control(
			'hide_testimonials',
			array(
				'label'       => esc_html__( 'Hide testimonials', 'tevah' ),
				'description' => esc_html__( 'Check this if you want to hide testimonials from the Front Page Template.', 'tevah' ),
				'section'     => 'front-page',
				'priority'    => 70,
				'type'        => 'checkbox'
			)
		...
		$wp_customize->add_control(
			'order_testimonials',
			array(
				'label'       => esc_html__( 'Testimonials after featured area', 'tevah' ),
				'description' => esc_html__( 'Check this if you want to move testimonials after Featured area in the Front Page Template.', 'tevah' ),
				'section'     => 'front-page',
				'priority'    => 75,
				'type'        => 'checkbox'
			)
	...
	$front_page_featured_choices = array( 
		'child-pages' => esc_html__( 'Child Pages', 'tevah' ),
		'blog-posts'  => esc_html__( 'Blog Posts', 'tevah' ),
		'portfolios'  => esc_html__( 'Portfolios', 'tevah' )
	);
	...
	$wp_customize->add_control(
		'front_page_featured',
		array(
			'label'       => esc_html__( 'Featured Content', 'tevah' ),
			'description' => esc_html__( 'Select do you want to feature Child Pages, Blog Posts or Portfolios in Front Page.', 'tevah' ),
			'section'     => 'front-page',
			'priority'    => 80,
			'type'        => 'radio',
	...
	$wp_customize->add_control(
		'callout_image',
		array(
			'label'       => esc_html__( 'Callout Image', 'tevah' ),
			'description' => sprintf( __( '<a href="%s" target="_blank">Upgrade to Tevah (Pro)</a> for adding Callout Image.', 'tevah' ), esc_url( toivo_lite_get_upgrade_link() ) ),
			'section'     => 'front-page',
			'priority'    => 170,
			'type'        => 'text'
		)
	...
	$wp_customize->add_section(
		'background',
		array(
			'title'    => esc_html__( 'Background Settings', 'tevah' ),
			'priority' => 30,
			'panel'    => 'theme'
		)
	);
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize,
		'header_background_color',
		array(
			'label'       => esc_html__( 'Header Background Color', 'tevah' ),
			'section'     => 'background',
			'priority'    => 40,
		)
	) );
	...
	$wp_customize->add_control(
		'header_background_color_opacity',
			array(
				...
				'priority'    => 50,
				'section'     => 'background',
				'label'       => esc_html__( 'Header Color Opacity.', 'tevah' ),
				'description' => esc_html__( 'Set Header Color opacity.', 'tevah' ),
				'input_attrs' =>
					array(
						'min'   => 0,
						'max'   => 100,
	...
	$wp_customize->add_control(
		new Toivo_Lite_Customize_Control_Info_Text(
			...
			'callout_image',
				array(
					'label'       => esc_html__( 'Subsidiary Sidebar Background', 'tevah' ),
					'description' => sprintf( __( '<a href="%s" target="_blank">Upgrade to Tevah (Pro)</a> for adding Subsidiary Sidebar Background Image and Color.', 'tevah' ), esc_url( toivo_lite_get_upgrade_link() ) ),
					'section'     => 'background',
					'priority'    => 60,
					'type'        => 'info-text'
				)
	...
	if( post_type_exists( 'jetpack-portfolio' ) ) {
		...
		$wp_customize->add_section(
			'portfolio',
			array(
				'title'    => esc_html__( 'Portfolio Settings', 'tevah' ),
				'priority' => 40,
				'panel'    => 'theme'
			)
		);
		...
		$wp_customize->add_control(
			new Tevah_Lite_Customize_Control_Info_Text(
				$wp_customize,
				'portfolio_title',
					array(
						'label'       => esc_html__( 'Portfolio Page Title', 'tevah' ),
						'description' => sprintf( __( '<a href="%s" target="_blank">Upgrade to Tevah (Pro)</a> for changing Portfolio title and description.', 'tevah' ), esc_url( toivo_lite_get_upgrade_link() ) ),
						'section'     => 'portfolio',
						'type'        => 'info-text'
					)
				)
	...
	$wp_customize->add_section(
		'footer',
		array(
			'title'    => esc_html__( 'Footer Settings', 'tevah' ),
			'priority' => 50,
			'panel'    => 'theme'
		)
	);
	...
	$wp_customize->add_control(
		new Toivo_Lite_Customize_Control_Info_Text(
			...
			'footer_section',
				array(
					'label'       => esc_html__( 'Enter footer text', 'tevah' ),
					'description' => sprintf( __( '<a href="%s" target="_blank">Upgrade to Tevah (Pro)</a> for hiding or changing Footer text.', 'tevah' ), esc_url( toivo_lite_get_upgrade_link() ) ),
					'section'     => 'footer',
					'type'        => 'info-text'
				)
			)
...
function toivo_lite_color_backgrounds_css() {
	...
	wp_add_inline_style( 'tevah-style', $bg_color_css );
}
...
function toivo_lite_customize_preview_js() {
	wp_enqueue_script( 'tevah-customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), TOIVO_LITE_VERSION, true );
}

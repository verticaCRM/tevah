...
class X2SummationReport extends X2Report {
    ...
    public function generate () {
        ...
        $selectClause = $this->buildSelectClause (
            array_merge ($this->getGroupAttrs (), $this->aggregates), !$this->export);
        ...
        if ($this->print || $this->email || $this->export) {
            ...
            $records = $this->insertGroupHeaders (
                $drillDownRecords, $this->formatData ($groupHeaders), $drillDownFormattedHeaders,
                !$this->export);
    ...
    private function insertGroupHeaders (
        ...
        if ($this->groupsOrderBy === array()) {
            ...
            return $this->fastGroupHeaderInsertion (
                $drillDownRecords, $groupHeaders, $drillDownFormattedHeaders, $insertHeaderTokens);
        }
    ...
    private function getGroupByClause () {
        $groupByClause = '';
        if (count ($this->groups)) $groupByClause .= 'GROUP BY ';
        for ($i = 0; $i < count ($this->groups); $i++) {
            $groupAttr = $this->groups[$i][0];
            $groupDirection = $this->groups[$i][1];

...
$menuItems = array(
    ...
    array('label' => Yii::t('services', 'Case Report')),
);
...
$itemModel = new Services;
$criteriaArr = array();
if(!empty($_GET)){
    $_SESSION['serviceReport']['GET'] = json_encode($_GET, false);
}else{
    $_SESSION['serviceReport']['GET'] = '';
}
if(isset($_GET['Services'])){
    $filterAttributes = $_GET['Services'];
...
function createAttributeCell() {
	...
	div.attr('class', 'cell');
	var label = $('<label for=\"Services[attribute][]\">Attribute</label>');
	var dropdown = createDropdown(attributeLabels);
	dropdown.attr('name', 'Services[attribute][]').attr('onchange','updateForm(this);');
...
function createComparisonCell(field) {
	...
	var div = $(document.createElement('div'));
	div.attr('class', 'cell');
	var label = $('<label for=\"Services[comparison][]\">Comparison</label>');
	var dropdown = createDropdown(comparisonList, ignoreList);
	dropdown.attr('name', 'Services[comparison][]');
...
function createValueCell(field) {
	...
	switch(fieldTypes[field]) {
		...
		case 'visibility':
			...
			input.attr('name', 'Services[value][]');
		...
		case 'link':
			...
			hidden.attr('name', 'Services[value][]');
			input = $('<input size=\"30\" type=\"text\" value=\"\">');
            input.blur(function(){
	...
	var div = $(document.createElement('div'));
	div.attr('class', 'cell');
	var label = $('<label for=\"Services[value][]\">Value</label>');
...
?>
<div class="page-title"><h2><?php echo Yii::t('services', 'Services Report'); ?></h2></div>
<div class="form">
    <?php
    $form = $this->beginWidget('CActiveForm', array(
        'action' => 'servicesReport',
        'id' => 'servicesReport',
    ...
    <div class="row">
        <div class="cell">
            <?php echo CHtml::label(Yii::t('services', 'Date Field'), 'dateField'); ?>
            <?php
        ...
        <div class="cell">
            <a href="#" class="x2-button" id="filters-link" style="margin-top:15px;"><?php echo Yii::t('services','Advanced Filters');?></a>
        </div>
        <div class="right">
            <a href="#" id="export-button" class="x2-button" style="margin-top:15px;"><?php echo Yii::t('services','Export Data');?></a>
        </div>
    </div>
    <div class="row">
...
<div id="report-data">
	...
	if(!empty($dataProvider)){
		$this->widget('X2GridView', array(
			...
			'template'=> '<div class="page-title icon services rounded-top">{title}{buttons}{summary}</div>{items}{pager}',
...
<div id="export-wrapper" class="form" style="display:none;">
    ...
    <div style="display:none" id="download-link-box">
        <?php echo Yii::t('services', 'Please click the link below to download this report.'); ?><br><br>
        <a class="x2-button" id="download-link" href="#"><?php echo Yii::t('app', 'Download'); ?>!</a>
    </div>
</div>
<script>
    $('#export-button').on('click',function(){
        exportServiceReport(0);
    });
    function exportServiceReport(page){
        if($('#export-status').length==0){
            $('#report-data').hide();
            $('#export-wrapper').show();
            $('#status-text').append("<div id='export-status'>Exporting <b>Service Case</b> data...<br></div>");
        }
        $.ajax({
            url:'exportServiceReport?page='+page,
            success:function(data){
                if(data>0){
                    $('#export-status').html(((data)*100)+" records from <b>Service Cases</b> successfully exported.<br>");
                    exportServiceReport(data);
                }else{
    ...
    $('#download-link').click(function(e) {
        ...
        window.location.href = '<?php echo $this->createUrl('/admin/downloadData', array('file' => $_SESSION['serviceReport']['serviceReportFile'])); ?>';
    });</script>

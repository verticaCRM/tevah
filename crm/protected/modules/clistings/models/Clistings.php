 ...
 */
class Clistings extends X2Model {
	 ...
	 */
	public function tableName() { return 'x2_clistings'; }
    ...
    public function renderInput($fieldName, $htmlOptions = array()) {
		...
		if("c_currency_id" === $fieldName){
      if (!$field) {
          return $this->renderErroneousField();
      }
		$this->currency = $this->c_currency_id;
		$settings = CActiveRecord::model('Admin')->findByPk(1);
//CActiveRecord::model('Admin')->setAttribute('currency',$this->c_currency_id);
//getAttribute
$settings->setAttribute('currency',$this->c_currency_id);

# I need to see what this is here: ApplicationConfigBehavior

//echo CVarDumper::dump(CActiveRecord::model('Admin'));
//echo CVarDumper::dump($this);
		}
...
//        $data["currency"] = $this->getField($data["c_currency_id"]);
  //echo CVarDumper::dump($this);      

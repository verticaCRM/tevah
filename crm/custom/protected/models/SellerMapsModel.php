...
class SellerMapsModel extends CModel {
    ...
    public function getReleaseStatus () {
        if (!isset ($this->relatedModel)) return null;
        return $this->c_release_status;
    }

    public function getHiddenStatus () {
        if (!isset ($this->relatedModel)) return null;
        return $this->c_is_hidden;
    }

    public function getListingCity () {
        if (!isset ($this->relatedModel)) return null;
        return $this->c_listing_city_c;
    }
    ...
    public function renderAttribute ($name) {
        switch ($name) {
            ...
            case 'nameId':
                ...
                break;
            case 'assignedTo':
                echo $this->renderAttribute ('assignedTo');
                break;
            case 'c_release_status':
                echo $this->renderAttribute ('c_release_status');
                break;
            case 'c_date_released':
                echo $this->renderAttribute ('c_date_released');
                break;
            case 'c_is_hidden':
                echo $this->renderAttribute ('c_is_hidden');
                break;
            case 'description':
                echo $this->renderAttribute ('description');
                break;
            case 'c_listing_city_c':
                echo $this->renderAttribute ('c_listing_city_c');
                break;
            case 'c_listing_town_c':
                echo $this->renderAttribute ('c_listing_town_c');
                break;
            case 'c_listing_region_c':
                echo $this->renderAttribute ('c_listing_region_c');
                break;
            case 'c_financial_net_cashflow_c':
                echo $this->renderAttribute ('c_financial_net_cashflow_c');
                break;
            case 'c_listing_askingprice_c':
                echo $this->renderAttribute ('c_listing_askingprice_c');
                break;
            case 'c_ownerscashflow':
                echo $this->renderAttribute ('c_ownerscashflow');
                break;
            case 'c_businesscategories':
                echo $this->renderAttribute ('c_businesscategories');
                break;
            case 'c_sales_stage':
                echo $this->renderAttribute ('c_sales_stage');
                break;
            case 'c_listing_id':
                echo $this->renderAttribute ('c_listing_id');
                break;
            case 'c_name_dba_c':
                echo $this->renderAttribute ('c_name_dba_c');
                break;
            case 'createDate':
                echo X2Html::dynamicDate ($this->createDate);
                break;
        }
    }
            ...
            foreach ($filters as $filter) {
                ...
                switch ($filter) {
                    ...
                    case 'assignedTo':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('assignedTo.fullName'));
                        break;
                    case 'c_is_hidden':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_is_hidden'));
                        break;
                    case 'c_listing_id':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_listing_id'));
                        break;
                    case 'c_release_status':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_release_status'));
                        break;
                    case 'c_listing_city_c':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_listing_city_c'));
                        break;
                    case 'c_listing_town_c':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_listing_town_c'));
                        break;
                    case 'c_listing_region_c':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_listing_region_c'));
                        break;
                    case 'c_financial_net_cashflow_c':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_financial_net_cashflow_c'));
                        break;
                    case 'c_listing_askingprice_c':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_listing_askingprice_c'));
                        break;
                    case 'c_ownerscashflow':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_ownerscashflow'));
                        break;
                    case 'c_name_dba_c':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_name_dba_c'));
                        break;
                    case 'c_sales_stage':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getAttribute ('c_sales_stage'));
                        break;
                    case 'label':
                        $filterOut = !preg_match (
                            '/'.$val.'/i',
                            $model->getRelationshipLabel (
                                $this->myModel->id, get_class ($this->myModel)));
                        break;
                    case 'createDate':
                        $timestampA = Formatter::parseDate ($val);
                        $timestampB = Formatter::parseDate (
                            $model->getAttribute ('createDate'));
